// // content.js — Elaborative Interrogation Study
// // Injected into semanticscholar.org/reader/* pages.
...
// // ─── CONTEXT-SAFE CHROME API WRAPPERS ────────────────────────────────────────
// // "Extension context invalidated" happens when the extension is reloaded while
// // a content script is still running. Every chrome.* call throws synchronously.
// // These wrappers catch that error and fall back gracefully.

// function isContextValid() {
//   try { return !!chrome.runtime?.id; } catch(_) { return false; }
// }

// // Safe chrome.storage.local.set — falls back to localStorage
...
//       <label class="rs-radio"><input type="radio" name="condition" value="baseline" /><span>Baseline</span></label>
//       <label class="rs-radio"><input type="radio" name="condition" value="frictionless" /><span>Frictionless</span></label>
//       <label class="rs-radio"><input type="radio" name="condition" value="friction" /><span>Productive Friction</span></label>
//     </div>
...
// // ─── AUTOSAVE ─────────────────────────────────────────────────────────────────
// // Writes SESSION to chrome.storage.local every 30s and on tab hide/unload.
// // This is the safety net — if the student never clicks "End Session", the data
// // is here and will be offered for recovery on the next load.
...
//   // Save immediately so even a very short session leaves a trace
//   save();
// }
...
// // ─── SESSION SUBMISSION ───────────────────────────────────────────────────────
// // Primary path: Google Forms (no auth, instant, appears in linked Sheet).
...
//       {
//         type: "uploadToDrive",
//         filename: (SESSION.sessionId || "session") + ".json",
//         content: jsonStr,
//         sessionJson: jsonStr,
//         participantId: SESSION.participantName + "_" + SESSION.sessionId,
...
//           clearAutosave(); // safe to clear — it's in the Sheet
//           showToast("Session submitted to Google Forms ✓", false, 4000);
//         } else {
//           // Submission failed — keep storage, show fallback UI
...
//           showToast("Submission failed — data preserved. See options below.", true, 8000);
//         }
//       }
//     );
//   });
  ...
  btn.addEventListener("click", () => {
    ...
    function onDriveResponse(response) {
      if (response?.ok) {
        ...
        btn.style.background = "#b45309";
        showSubmissionFailedUI(SESSION, response?.error || "unknown");
        showToast("Drive save failed — data preserved in browser. See options below.", true, 8000);
      }
    }
